...
int nthreads;
scene_t scene;
float pixelz [width][height][3];
/*
Put the creating Image code in a function
to multithread it
 */
void* createImage(void* arg){
  ...
  const double pixel_dy = tan( 0.5*camera_fov ) / ((double)height*0.5);
    ...
    /* for every pixel */
    for( int px=*(int*) arg; px<width; px+=nthreads )//updated multithread
      {
        ...
        for( int py=0; py<height; ++py )
	  {
	    ...
	    Vec3 pixel_color;
	      ...
	      scaled_color[i] = max( min(scaled_color[i], 255), 0);
            ...
            /* write this pixel out to disk. ppm is forgiving about whitespace,                                                             * but has a maximum of 70 chars/line, so use one line per pixel                                                               */
	    pixelz[px][py][0] =scaled_color[0];
	    pixelz[px][py][1] =scaled_color[1];
	    pixelz[px][py][2] =scaled_color[2];
 	  }
       }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    */
    ...
    printf( "# Rendering scene with %d spheres and %d lights\n",
            scene.sphere_count,
            scene.light_count );
    pthread_t* threadId = (pthread_t*) malloc(nthreads*sizeof(pthread_t));//allocate
    int ID[nthreads];
    int i;
    for(i=0;i< nthreads; i++){//make the id's and threads
      ID[i]=i;
      pthread_create(&threadId[i],NULL,createImage, &ID[i]);
    }

    for(i=0; i<nthreads; i++){//join threads
       int check =pthread_join(threadId[i], NULL);
       if(check){//join returns nonzero for failure
       fprintf(stderr,"Issue when joining threads");
       exit(1);
      }
    }
    int j;
    for( i=0; i<width; i++){ //Output
      for( j=0; j<height; j++){
	printf( "%.0f %.0f %.0f\n",pixelz[i][j][0], pixelz[i][j][1], pixelz[i][j][2] );
      }//0 digits after decimal
      printf("\n");
    }

  free(threadId);//good practice
  free_scene(&scene);
